...
//
static const uint16_t g_au16CRCTable[256] =
{
...
uint16_t util_CRC16Cal(uint8_t *pu8Source, uint16_t u16Len)
{
    ...
    for(i = 0; i < u16Len; i++)
    {
    	u16Result = (u16Result << 8) ^ (uint16_t)g_au16CRCTable[(u16Result >> 8) ^ pu8Source[i]];
    }
...
ErrorStatus util_calculate_checksum(uint8_t *pau8SrcData, uint32_t u32DataLen, uint8_t *pu8CheckSum)
{
	...
	u32Sum = 0;
	for(u32Dex=0; u32Dex<u32DataLen; u32Dex++)
	{
		u32Sum += *(pau8SrcData + u32Dex);
	}

	*pu8CheckSum = (uint8_t)(u32Sum & 0x000000FF);
...
ErrorStatus util_average(const uint32_t *pau32SrcData, uint32_t u32DataLen, uint32_t *pu32Average)
{
	...
	for(u32Dex=0; u32Dex<u32DataLen; u32Dex++)
	{
		u64Sum += *(pau32SrcData + u32Dex);
	}

	*pu32Average = (uint32_t)(u64Sum / u32DataLen);
...
ErrorStatus util_average_float(const float *pfSrcData, uint32_t u32DataLen, float *pfAverage)
{
	...
	for(u32Dex=0; u32Dex<u32DataLen; u32Dex++)
	{
		f64Sum += *(pfSrcData + u32Dex);
	}

	*pfAverage = (float)(f64Sum / u32DataLen);
...
void util_float_change_endtype(float *pfData)
{
	...
	pu8Temp = (uint8_t *)&fTemp;

	((uint8_t *)pfData)[0] = pu8Temp[3];
	((uint8_t *)pfData)[1] = pu8Temp[2];
	((uint8_t *)pfData)[2] = pu8Temp[1];
	((uint8_t *)pfData)[3] = pu8Temp[0];
}
//-------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------
