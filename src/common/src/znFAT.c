...
#include "inttypes.h"
#include "znFAT.h"
//#include "ss_sdcard.h"     //存储设备的扇区读写驱动，这里是SD卡
...
//全局变量定义
static struct znFAT_direntry g_stTempRec;
static int8_t g_ai8TempDirName[13];
static uint32_t g_u32TempDirCluster;
static uint32_t g_u32TempLastCluster;

static uint8_t g_au8SectorBuffer[512]; //扇区数据读写缓冲区,由外部提供


//extern struct znFAT_Init_Arg *pArg; //初始化参数结构体指针，用以指向某一存储设备的初始化参数结构体，由外部提供
//extern uint8_t g_u8DeviceNo;//设备号，外部变量


static struct znFAT_Init_Arg g_stznFATInitArg;
static struct znFAT_Init_Arg *pArg = &g_stznFATInitArg; //初始化参数结构体指针，用以指向某一存储设备的初始化参数结构体
static uint8_t g_u8DeviceNo = ZNFAT_SDCARD;//设备号
...
{
	switch(g_u8DeviceNo)	//由Dev_No来决定所使用的存储设备驱动
	{
	case ZNFAT_SDCARD:
		// SD_Reset();
		// SD_Init();
		SS_SDInit(&g_cstSDCardCfg);
		break;
	case ZNFAT_UDISK:
...
uint8_t znFAT_ReadSector(uint32_t addr,uint8_t *buf)
{
	switch(g_u8DeviceNo)	//由Dev_No来决定所使用的存储设备驱动
	{
	case ZNFAT_SDCARD:
		return SS_SDBlockRead(buf, addr);
...
uint8_t znFAT_WriteSector(uint32_t addr,uint8_t *buf)
{
	switch(g_u8DeviceNo)	//由Dev_No来决定所使用的存储设备驱动
	{
	case ZNFAT_SDCARD:
//		return SD_Write_Sector(addr,buf);
		return SS_SDBlockWrite(buf, addr);
		break;
	case ZNFAT_UDISK:
...
uint32_t LE2BE(uint8_t *dat,uint8_t len)
{
	uint32_t temp=0;
	uint32_t fact=1;
	uint8_t  i=0;
...
void znFAT_Empty_Cluster(uint32_t Cluster)
{
	uint32_t iSec;
	uint16_t i;
	for(i=0;i<pArg->BytesPerSector;i++)
	{
		g_au8SectorBuffer[i]=0;
	}
	for(iSec=ZNFAT_SOC(Cluster);iSec<ZNFAT_SOC(Cluster)+pArg->SectorsPerClust;iSec++)
	{
		znFAT_WriteSector(iSec,g_au8SectorBuffer);
	}
}
...
uint32_t znFAT_Get_Remain_Cap(void)
{
	znFAT_ReadSector(1+pArg->BPB_Sector_No,g_au8SectorBuffer);

	if(((struct znFAT_FSInfo *)g_au8SectorBuffer)->Free_Cluster[0]==0xff
	&& ((struct znFAT_FSInfo *)g_au8SectorBuffer)->Free_Cluster[1]==0xff
	&& ((struct znFAT_FSInfo *)g_au8SectorBuffer)->Free_Cluster[2]==0xff
	&& ((struct znFAT_FSInfo *)g_au8SectorBuffer)->Free_Cluster[3]==0xff)
	{
		return pArg->Total_Size;
	}
	else
	{
		return LE2BE(((struct znFAT_FSInfo *)g_au8SectorBuffer)->Free_Cluster,4)*pArg->SectorsPerClust*pArg->BytesPerSector;
	}
}
...
uint16_t znFAT_Find_DBR(void)
{
	uint16_t sec_dbr;

	znFAT_ReadSector(0,g_au8SectorBuffer);
	if(g_au8SectorBuffer[0]!=0xeb)
	{
		sec_dbr=LE2BE(((((struct znFAT_PartSector *)(g_au8SectorBuffer))->Part[0]).StartLBA),4);
	}
	else
	{
		sec_dbr=0;
...
uint32_t znFAT_Get_Total_Size(void)
{
	return pArg->Total_Size;
}
...
uint32_t Search_Last_Usable_Cluster(void)
{
	znFAT_ReadSector(1+pArg->BPB_Sector_No,g_au8SectorBuffer);
	return LE2BE(((struct znFAT_FSInfo *)g_au8SectorBuffer)->Last_Cluster,4);
}
...
void znFAT_Init(void)
{
	...
	bpb=(struct znFAT_BPB *)(g_au8SectorBuffer);               //将数据缓冲区指针转为struct znFAT_BPB 型指针

	pArg->DEV_No=g_u8DeviceNo; //装入设备号

	pArg->BPB_Sector_No   =znFAT_Find_DBR();               //znFAT_FindBPB()可以返回BPB所在的扇区号

	znFAT_ReadSector(pArg->BPB_Sector_No,g_au8SectorBuffer);
	...
	g_u32TempLastCluster=Search_Last_Usable_Cluster();
}
...
void znFAT_Update_FSInfo_Free_Clu(uint32_t PlusOrMinus)
{
	uint32_t Free_Clu=0;
	znFAT_ReadSector(1+pArg->BPB_Sector_No,g_au8SectorBuffer);
	...
	((struct znFAT_FSInfo *)g_au8SectorBuffer)->Free_Cluster[0]=Free_Clu&0x000000ff;//((uint8_t *)&Free_Clu)[0];
	((struct znFAT_FSInfo *)g_au8SectorBuffer)->Free_Cluster[1]=(Free_Clu&0x0000ff00)>>8;//((uint8_t *)&Free_Clu)[1];
	((struct znFAT_FSInfo *)g_au8SectorBuffer)->Free_Cluster[2]=(Free_Clu&0x00ff0000)>>16;//((uint8_t *)&Free_Clu)[2];
	((struct znFAT_FSInfo *)g_au8SectorBuffer)->Free_Cluster[3]=(Free_Clu&0xff000000)>>24;//((uint8_t *)&Free_Clu)[3];
	znFAT_WriteSector(1+pArg->BPB_Sector_No,g_au8SectorBuffer);
}
...
void znFAT_Update_FSInfo_Last_Clu(uint32_t Last_Clu)
{
	znFAT_ReadSector(1+pArg->BPB_Sector_No,g_au8SectorBuffer);
	((struct znFAT_FSInfo *)g_au8SectorBuffer)->Last_Cluster[0]=Last_Clu&0x000000ff;//((uint8_t *)&Last_Clu)[0];
	((struct znFAT_FSInfo *)g_au8SectorBuffer)->Last_Cluster[1]=(Last_Clu&0x0000ff00)>>8;//((uint8_t *)&Last_Clu)[1];
	((struct znFAT_FSInfo *)g_au8SectorBuffer)->Last_Cluster[2]=(Last_Clu&0x00ff0000)>>16;//((uint8_t *)&Last_Clu)[2];
	((struct znFAT_FSInfo *)g_au8SectorBuffer)->Last_Cluster[3]=(Last_Clu&0xff000000)>>24;//((uint8_t *)&Last_Clu)[3];
	znFAT_WriteSector(1+pArg->BPB_Sector_No,g_au8SectorBuffer);
}
...
uint32_t znFAT_GetNextCluster(uint32_t LastCluster)
{
	uint32_t temp;
	...
	znFAT_ReadSector(temp,g_au8SectorBuffer);
	pFAT=(struct znFAT_FAT *)g_au8SectorBuffer;
	pFAT_Item=&((pFAT->Items)[LastCluster%128]);

	return LE2BE((uint8_t *)pFAT_Item,4);
}
...
uint8_t Compare_Dir_Name(CONST int8_t *a,CONST int8_t *b)
{
	uint8_t i;
...
uint8_t FilenameMatch(int8_t *pat,int8_t *name)
{
	uint8_t match,ndone;
	int8_t *cpp,*cpn;
	...
	while(ndone)
	{
		...
		case '*':
			cpp++;
			cpn=(int8_t *)strchr((const char *)cpn,*cpp);
			if(cpn==NULL)
			{
				cpn=name;
				while(*cpn) cpn++;
...
void znFAT_toFileName(CONST int8_t *dName,int8_t *pName)
{
	uint8_t i=0;
	...
	strcpy((char *)(pName+i),(const char *)(pName+8));
}
...
void Str2Up(int8_t *str)
{
	uint8_t len=strlen((const char *)str),i;
...
{
	uint32_t Cur_Clust,sec_temp,iSec,iDir,Old_Clust;
	uint8_t i=1,counter=0,flag=0;
	struct znFAT_direntry *pDir;
	int8_t name[20];
	...
	else
	{
		while(path[i]!=0)
		{
			if(path[i]=='\\')
			{
				...
				do
				{
					sec_temp=(ZNFAT_SOC(Cur_Clust));
					for(iSec=sec_temp;iSec<sec_temp+pArg->SectorsPerClust;iSec++)
					{
						znFAT_ReadSector(iSec,g_au8SectorBuffer);
						for(iDir=0;iDir<pArg->BytesPerSector;iDir+=sizeof(struct znFAT_direntry))
						{
							pDir=((struct znFAT_direntry *)(g_au8SectorBuffer+iDir));
							if(Compare_Dir_Name((const int8_t *)pDir->deName,(const int8_t *)name) && (pDir->deAttributes&0x10))
							{
								flag=1;
				...
				if(!flag)
				{
					g_u32TempDirCluster=Old_Clust;
					strcpy((char *)g_ai8TempDirName,(const char *)name);
					flag=0;
					return 0;
				}
				flag=0;
...
/**************************************************************************
 ...
 - 隶属模块：znFAT文件系统模块
 - 函数属性：外部，使用户使用
 - 参数说明：pfi: znFAT_FileInfoStruct类型的结构体指针，用来装载文件的参数信息
...
{
	uint32_t Cur_Clust,sec_temp,iSec,iFile,iItem=0;
	uint8_t flag=0,i=0,h=0;
	//uint8_t index=0;
	struct znFAT_direntry *pFile;
	int8_t temp_file_name[30];
...
//	}
	...
	Cur_Clust=znFAT_Enter_Dir(filepath);
	if(Cur_Clust)
	{
		Str2Up(g_ai8TempDirName);
		do
		{
			sec_temp=ZNFAT_SOC(Cur_Clust);
			for(iSec=sec_temp;iSec<sec_temp+pArg->SectorsPerClust;iSec++)
			{
				znFAT_ReadSector(iSec,g_au8SectorBuffer);
				for(iFile=0;iFile<pArg->BytesPerSector;iFile+=sizeof(struct znFAT_direntry))
				{
					pFile=((struct znFAT_direntry *)(g_au8SectorBuffer+iFile));
					if(pFile->deAttributes&(is_file?0x20:0x10))
					{
						znFAT_toFileName((const int8_t *)pFile->deName,temp_file_name);
						if(is_file)
						{
							i=0;
							while(g_ai8TempDirName[i]!=0)
							{
								if(g_ai8TempDirName[i++]=='.')
									h=1;
							}
							if(!h)
							{
								g_ai8TempDirName[i]='.';
								g_ai8TempDirName[i+1]=0;
							}
						}
						if(FilenameMatch(g_ai8TempDirName,temp_file_name) \
								&& (pFile->deName[0]!=0xe5) \
								&& ((pFile->deAttributes)&(is_file?0x20:0x10))) //匹配文件名
						{
							if(item==iItem)
							{
								flag=1;
								...
								temp_file_name[strlen((const char *)temp_file_name)-1]=0;

								strcpy((char *)pfi->FileName,(const char *)temp_file_name);
								pfi->FileStartCluster=LE2BE(pFile->deLowCluster,2)+LE2BE(pFile->deHighClust,2)*65536;
								pfi->FileCurCluster=pfi->FileStartCluster;
								pfi->FileCurSector=ZNFAT_SOC(pfi->FileStartCluster);
								pfi->FileCurPos=0;
								pfi->FileCurOffset=0;
								pfi->Rec_Sec=iSec;
								pfi->nRec=iFile;
...
/**************************************************************************
 ...
 - 隶属模块：znFAT文件系统模块
 - 函数属性：外部，使用户使用
 - 参数说明：pfi:znFAT_FileInfoStruct类型的结构体指针，用于装载文件参数信息，文件
...
uint8_t znFAT_Seek_File(struct znFAT_FileInfoStruct *pfi,uint32_t offset)
{
	...
	if(offset<=pfi->FileSize)
	{
		...
		{
			pfi->FileCurCluster=pfi->FileStartCluster;
			pfi->FileCurSector=ZNFAT_SOC(pfi->FileCurCluster);
			pfi->FileCurPos=0;
			pfi->FileCurOffset=0;
		}
		//目标偏移量与文件当前偏移量所差的字节数不小于文件在当前扇区中的位置到扇区末尾的字节数
		if((offset-pfi->FileCurOffset)>=(pArg->BytesPerSector-pfi->FileCurPos))
		{
			pfi->FileCurOffset+=(pArg->BytesPerSector-pfi->FileCurPos);
			pfi->FileCurPos=0;
			if(pfi->FileCurSector-ZNFAT_SOC(pfi->FileCurCluster)==(pArg->SectorsPerClust-1))
			{
		...
		temp=ZNFAT_SOC(pfi->FileCurCluster);
		if((offset-(pfi->FileCurOffset))/pArg->BytesPerSector+(pfi->FileCurSector-temp)>(pArg->SectorsPerClust-1))
		{
			pfi->FileCurOffset+=(((pArg->SectorsPerClust)-(pfi->FileCurSector-(ZNFAT_SOC(pfi->FileCurCluster))))*pArg->BytesPerSector);
			pfi->FileCurCluster=znFAT_GetNextCluster(pfi->FileCurCluster);
			pfi->FileCurSector=ZNFAT_SOC(pfi->FileCurCluster);
			pfi->FileCurPos=0;
		}
		...
		u32Temp = ZNFAT_SOC(pfi->FileCurCluster);
		pfi->FileCurSector=(u32Temp)+(offset-pfi->FileCurOffset)/pArg->BytesPerSector;
		pfi->FileCurPos=((offset-pfi->FileCurOffset))%pArg->BytesPerSector;
		pfi->FileCurOffset=offset;
	}
	else
...
//		znFAT_Seek_File(pfi,offset);
		...
		znFAT_ReadSector(pfi->FileCurSector,g_au8SectorBuffer);
		for(i=pfi->FileCurPos;i<pArg->BytesPerSector;i++)
		{
			...
			pbuf[counter]=g_au8SectorBuffer[i];
			counter++;
			pfi->FileCurPos++;
			pfi->FileCurOffset++;
		}
		if(pfi->FileCurSector-(ZNFAT_SOC(pfi->FileCurCluster))!=(pArg->SectorsPerClust-1))
		{
			for(j=pfi->FileCurSector+1;j<(ZNFAT_SOC(pfi->FileCurCluster))+pArg->SectorsPerClust;j++)
			{
				znFAT_ReadSector(j,g_au8SectorBuffer);
				pfi->FileCurSector=j;
				for(i=0;i<pArg->BytesPerSector;i++)
				{
					if(counter>=len)
		...
		{
		pfi->FileCurCluster=znFAT_GetNextCluster(pfi->FileCurCluster);
		for(j=(ZNFAT_SOC(pfi->FileCurCluster));j<(ZNFAT_SOC(pfi->FileCurCluster))+pArg->SectorsPerClust;j++)
		{
			znFAT_ReadSector(j,g_au8SectorBuffer);
			pfi->FileCurSector=j;
			for(i=0;i<pArg->BytesPerSector;i++)
		...
		pfi->FileCurCluster=znFAT_GetNextCluster(pfi->FileCurCluster);
		temp=(ZNFAT_SOC(pfi->FileCurCluster))+((len-counter)/pArg->BytesPerSector);
		pfi->FileCurSector=(ZNFAT_SOC(pfi->FileCurCluster));
		for(j=(ZNFAT_SOC(pfi->FileCurCluster));j<temp;j++)
		{
		znFAT_ReadSector(j,g_au8SectorBuffer);
		pfi->FileCurSector=j;
		for(i=0;i<pArg->BytesPerSector;i++)
...
uint32_t znFAT_Find_Free_Clust(uint8_t flag)
{
	uint32_t iClu,iSec;
	struct znFAT_FAT *pFAT;
	for(iSec=pArg->FirstFATSector+g_u32TempLastCluster/128;iSec<pArg->FirstFATSector+pArg->FATsectors;iSec++)
	{
		znFAT_ReadSector(iSec,g_au8SectorBuffer);
		pFAT=(struct znFAT_FAT *)g_au8SectorBuffer;
		for(iClu=0;iClu<pArg->BytesPerSector/4;iClu++)
		{
			if(LE2BE((uint8_t *)(&((pFAT->Items))[iClu]),4)==0)
			{
				if(!flag)
				{
					znFAT_Update_FSInfo_Free_Clu(0);
					g_u32TempLastCluster=128*(iSec-pArg->FirstFATSector)+iClu;
					znFAT_Empty_Cluster(g_u32TempLastCluster);
					return g_u32TempLastCluster;
				}
				else
				{
...
/**************************************************************************
 ...
 - 隶属模块：znFAT文件系统模块
 - 函数属性：内部
 - 参数说明：prec:指向一个znFAT_direntry类型的结构体，它的结构就是znFAT中文件/
             目录项的结构
             name:文件或目录的名称
...
void Fill_Rec_Inf(struct znFAT_direntry *prec,int8_t *name,uint8_t is_dir,uint8_t *ptd)
{
	uint8_t i=0,len=0,h=0;
	uint16_t temp;
	...
	temp=ZNFAT_MAKE_FILE_TIME(ptd[3],ptd[4],ptd[5]);
	(prec->deCTime)[0]=temp;
	(prec->deCTime)[1]=temp>>8;
	temp=ZNFAT_MAKE_FILE_DATE(ptd[0],ptd[1],ptd[2]);
	(prec->deCDate)[0]=temp;
...
void znFAT_Modify_FAT(uint32_t cluster,uint32_t dat)
{
	znFAT_ReadSector(pArg->FirstFATSector+(cluster*4/pArg->BytesPerSector),g_au8SectorBuffer);
	g_au8SectorBuffer[((cluster*4)%pArg->BytesPerSector)+0]=dat&0x000000ff;
	g_au8SectorBuffer[((cluster*4)%pArg->BytesPerSector)+1]=(dat&0x0000ff00)>>8;
	g_au8SectorBuffer[((cluster*4)%pArg->BytesPerSector)+2]=(dat&0x00ff0000)>>16;
	g_au8SectorBuffer[((cluster*4)%pArg->BytesPerSector)+3]=(dat&0xff000000)>>24;
	znFAT_WriteSector(pArg->FirstFATSector+(cluster*4/pArg->BytesPerSector),g_au8SectorBuffer);

	znFAT_ReadSector(pArg->FirstFATSector+pArg->FATsectors+(cluster*4/pArg->BytesPerSector),g_au8SectorBuffer);
	g_au8SectorBuffer[((cluster*4)%pArg->BytesPerSector)+0]=dat&0x000000ff;
	g_au8SectorBuffer[((cluster*4)%pArg->BytesPerSector)+1]=(dat&0x0000ff00)>>8;
	g_au8SectorBuffer[((cluster*4)%pArg->BytesPerSector)+2]=(dat&0x00ff0000)>>16;
	g_au8SectorBuffer[((cluster*4)%pArg->BytesPerSector)+3]=(dat&0xff000000)>>24;
	znFAT_WriteSector(pArg->FirstFATSector+pArg->FATsectors+(cluster*4/pArg->BytesPerSector),g_au8SectorBuffer);
}
...
/**************************************************************************
 ...
 - 隶属模块：znFAT文件系统模块
 - 函数属性：内部
 - 参数说明：pfi:指向znFAT_FileInfoStruct类型的结构体，用于装载刚创建的文件的信息
...
uint8_t znFAT_Create_Rec(struct znFAT_FileInfoStruct *pfi,uint32_t cluster,int8_t *name,uint8_t is_dir,uint8_t *ptd)
{
	uint32_t iSec,iRec,temp_sec,temp_clu,new_clu,i,old_clu;
	uint8_t flag=0;
	uint16_t temp_Rec;
	struct znFAT_direntry *pRec;

	Fill_Rec_Inf(&g_stTempRec,name,is_dir,ptd);

	do
	{
		old_clu=cluster;
		temp_sec=ZNFAT_SOC(cluster);
		for(iSec=temp_sec;iSec<temp_sec+pArg->SectorsPerClust;iSec++)
		{
			znFAT_ReadSector(iSec,g_au8SectorBuffer);
			for(iRec=0;iRec<pArg->BytesPerSector;iRec+=sizeof(struct znFAT_direntry))
			{
				pRec=(struct znFAT_direntry *)(g_au8SectorBuffer+iRec);
				if((pRec->deName)[0]==0)
				{
					flag=1;
					if(is_dir)
					{
						...
						(g_stTempRec.deHighClust)[0]=(new_clu&0x00ff0000)>>16;
						(g_stTempRec.deHighClust)[1]=(new_clu&0xff000000)>>24;
						(g_stTempRec.deLowCluster)[0]=(new_clu&0x000000ff);
						(g_stTempRec.deLowCluster)[1]=(new_clu&0x0000ff00)>>8;
					}
					znFAT_ReadSector(iSec,g_au8SectorBuffer);
					for(i=0;i<sizeof(struct znFAT_direntry);i++)
					{
						((uint8_t *)pRec)[i]=((uint8_t *)(&g_stTempRec))[i];
					}
					znFAT_WriteSector(iSec,g_au8SectorBuffer);
					temp_sec=iSec;
					temp_Rec=iRec;
	...
	if(!flag)
	{
		...
		temp_sec=ZNFAT_SOC(temp_clu);
		temp_Rec=0;
		znFAT_ReadSector(temp_sec,g_au8SectorBuffer);
		if(is_dir)
		{
			if(!(new_clu=znFAT_Find_Free_Clust(0))) return -1;
		...
		for(i=0;i<sizeof(struct znFAT_direntry);i++)
		{
			g_au8SectorBuffer[i]=((uint8_t *)(&g_stTempRec))[i];
		}
		znFAT_WriteSector(temp_sec,g_au8SectorBuffer);
	}
	if(is_dir)
	{
		znFAT_Empty_Cluster(new_clu);

		Fill_Rec_Inf(&g_stTempRec,".",1,ptd);
		(g_stTempRec.deHighClust)[0]=(new_clu&0x00ff0000)>>16;
		(g_stTempRec.deHighClust)[1]=(new_clu&0xff000000)>>24;
		(g_stTempRec.deLowCluster)[0]=(new_clu&0x000000ff);
		(g_stTempRec.deLowCluster)[1]=(new_clu&0x0000ff00)>>8;
		...
		{
			(g_stTempRec.deHighClust)[0]=0;
			(g_stTempRec.deHighClust)[1]=0;
			(g_stTempRec.deLowCluster)[0]=0;
			(g_stTempRec.deLowCluster)[1]=0;
		}
		else
		{
			(g_stTempRec.deHighClust)[0]=(cluster&0x00ff0000)>>16;
			(g_stTempRec.deHighClust)[1]=(cluster&0xff000000)>>24;
			(g_stTempRec.deLowCluster)[0]=(cluster&0x000000ff);
			(g_stTempRec.deLowCluster)[1]=(cluster&0x0000ff00)>>8;
		}

		for(i=sizeof(struct znFAT_direntry);i<2*sizeof(struct znFAT_direntry);i++)
		{
			g_au8SectorBuffer[i]=((uint8_t *)(&g_stTempRec))[i-sizeof(struct znFAT_direntry)];
		}
		for(;i<pArg->BytesPerSector;i++)
		{
			g_au8SectorBuffer[i]=0;
		}
		temp_sec=ZNFAT_SOC(new_clu);
		znFAT_WriteSector(temp_sec,g_au8SectorBuffer);
	}
	else
	{
		strcpy((char *)pfi->FileName,(const char *)name);
		...
		pfi->FileAttr=g_stTempRec.deAttributes;
	}
...
{
	uint32_t i=0,counter=0,iSec,iClu;
	uint32_t temp_sub,temp_file_size,new_clu,temp_sec;
	struct znFAT_direntry *prec;
	if(len>0)
	{
		znFAT_ReadSector(pfi->Rec_Sec,g_au8SectorBuffer);
		prec=(struct znFAT_direntry *)(g_au8SectorBuffer+pfi->nRec);
		...
		if(!temp_file_size)
		{
			...
			znFAT_WriteSector(pfi->Rec_Sec,g_au8SectorBuffer);
		}
		else
		{
		...
		for(i=pfi->FileCurPos;i<pArg->BytesPerSector;i++)
		{
			g_au8SectorBuffer[i]=pbuf[counter];
			counter++;
			if(counter>=len)
			{
				iSec=pfi->FileCurSector;
		...
		znFAT_WriteSector(pfi->FileCurSector,g_au8SectorBuffer); //数据接缝

		if(pfi->FileCurSector-(ZNFAT_SOC(pfi->FileCurCluster))<(pArg->SectorsPerClust-1)) //判断是不是一个簇的最后一个扇区,先将当前簇所有扇区填满
		{
			for(iSec=pfi->FileCurSector+1;iSec<=(ZNFAT_SOC(pfi->FileCurCluster)+pArg->SectorsPerClust-1);iSec++)
			{
				for(i=0;i<pArg->BytesPerSector;i++)
				{
					g_au8SectorBuffer[i]=pbuf[counter];
		...
		znFAT_WriteSector(iSec,g_au8SectorBuffer);
		znFAT_ReadSector(pfi->Rec_Sec,g_au8SectorBuffer);
		(((struct znFAT_direntry *)(g_au8SectorBuffer+pfi->nRec))->deFileSize)[0]=((temp_file_size+len)&0x000000ff);
		(((struct znFAT_direntry *)(g_au8SectorBuffer+pfi->nRec))->deFileSize)[1]=((temp_file_size+len)&0x0000ff00)>>8;
		(((struct znFAT_direntry *)(g_au8SectorBuffer+pfi->nRec))->deFileSize)[2]=((temp_file_size+len)&0x00ff0000)>>16;
		(((struct znFAT_direntry *)(g_au8SectorBuffer+pfi->nRec))->deFileSize)[3]=((temp_file_size+len)&0xff000000)>>24;
		znFAT_WriteSector(pfi->Rec_Sec,g_au8SectorBuffer);
...
{
	struct znFAT_FileInfoStruct *pfi;

	while(!znFAT_Enter_Dir(dirpath))
	{
		if(znFAT_Create_Rec(pfi,g_u32TempDirCluster,g_ai8TempDirName,1,ptd)==1)
		{
			return 1;
		}
	}
...
/**************************************************************************
 ...
 - 隶属模块：znFAT文件系统模块
 - 函数属性：外部，使用户使用
 - 参数说明：pfi:一个指向znFAT_FileInfoStruct类型的结构体的指针，用来装载新创建的
...
uint8_t znFAT_Create_File(struct znFAT_FileInfoStruct *pfi,CONST int8_t *filepath,uint8_t *ptd)
{
	if(znFAT_Open_File(pfi,filepath,0,1))
	{
		...
		{
			if(znFAT_Create_Rec(pfi,g_u32TempDirCluster,g_ai8TempDirName,0,ptd)==1)
			{
				return 3;
			}
		}
...
{
	uint32_t cur_clu,next_clu;
	struct znFAT_FileInfoStruct fi;

	if(znFAT_Open_File(&fi,filepath,0,1))
	{
		return 1;
	}
	znFAT_ReadSector(fi.Rec_Sec,g_au8SectorBuffer);
	*(g_au8SectorBuffer+fi.nRec)=0xe5;
	znFAT_WriteSector(fi.Rec_Sec,g_au8SectorBuffer);

	cur_clu=fi.FileStartCluster;
	if(cur_clu)
	{
...
{
	struct znFAT_FileInfoStruct FileInfo2,FileInfo1;
	struct znFAT_Init_Arg *pArg_temp;
	uint32_t i;

	pArg_temp=pArg; //把pArg原来的指向赋给中间变量

	pArg=pArg2;g_u8DeviceNo=pArg->DEV_No; //标定目标存储设备
	if(znFAT_Create_File(&FileInfo1,tfilename,pt)) return 1; //在目标存储设备上创建目标文件

	pArg=pArg1;g_u8DeviceNo=pArg->DEV_No; //标定源存储设备
...
uint8_t znFAT_Rename_File(CONST int8_t *filename,CONST int8_t *newfilename)
{
	...
	if(znFAT_Open_File(&fi,filename,0,1))
		return 1; //文件打开失败

	znFAT_ReadSector(fi.Rec_Sec,g_au8SectorBuffer);

	for(i=0;i<11;i++)
		(g_au8SectorBuffer+fi.nRec)[i]=0x20;

	i=0;
	while(newfilename[i]!='.')
	{
		(g_au8SectorBuffer+fi.nRec)[i]=L2U(newfilename[i]);
		i++;
	}
	...
	while(newfilename[i])
	{
		(g_au8SectorBuffer+fi.nRec+8)[j]=L2U(newfilename[i]);
		i++;j++;
	}
...
uint8_t znFAT_File_Close(struct znFAT_FileInfoStruct *pfi)
{
	...
	for(i=0;i<sizeof(struct znFAT_FileInfoStruct);i++)
	{
		((uint8_t *)pfi)[i]=0;
	}
