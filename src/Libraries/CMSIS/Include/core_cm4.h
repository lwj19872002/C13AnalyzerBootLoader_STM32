...
__STATIC_INLINE void SysTick_Disable_Int(void)
{
	uint32_t	u32Temp;

	u32Temp = SysTick->CTRL;
	SysTick->CTRL	=	u32Temp & (~((uint32_t)SysTick_CTRL_TICKINT_Msk));
}

__STATIC_INLINE void SysTick_Enable_Int(void)
{
	uint32_t	u32Temp;

	u32Temp = SysTick->CTRL;
	SysTick->CTRL	=	u32Temp | SysTick_CTRL_TICKINT_Msk;
}
